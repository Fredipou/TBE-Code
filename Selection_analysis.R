...
mean_survival = mean(Recolte_foret$survie_larve, na.rm = T)
mean_survival

## Same but without the Pupae stage because there seems to be very little parasitism

mean_survival_nopupae = mean(Recolte_foret[Recolte_foret$stade != "Pupae",]$survie_larve, na.rm = T)
...
Recolte_foret <- Recolte_foret %>%
  group_by(stade.c) %>%
  mutate(rel_fitness = survie_larve / mean(survie_larve, na.rm = T)) %>%
  ungroup()
